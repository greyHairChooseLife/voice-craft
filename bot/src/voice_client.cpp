...
VoiceClient::VoiceClient(std::string host, uint16_t port)
...
void VoiceClient::poll(const std::function<void(const std::string&)>& on_line)
{
  if (!wsa_ready_)
    return;
...
void VoiceClient::start_connect()
{
  ...
  // inet_pton 은 _WIN32_WINNT>=0x0600 필요 (프로젝트는 0x0501). localhost IPv4
  // 점표기 주소엔 inet_addr 로 충분.
...
void VoiceClient::check_connect()
{
...
void VoiceClient::recv_lines(const std::function<void(const std::string&)>& on_line)
{
  char chunk[1024];
  for (;;)
  {
    ...
    if (n > 0)
    {
      ...
      continue;  // 더 읽을 게 있으면 계속 (한 프레임에 비울 수 있는 만큼)
    }
    if (n == 0)
    {
